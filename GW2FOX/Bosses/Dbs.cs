...
namespace GW2FOX
{
    public partial class Dbs : BaseForm
    {
        ...
        private void InitializeItemPriceTextBox()
        {
            ...
            DBSCost.Text = "Item-Preis: Wird geladen...";
            DBSCost.AutoSize = true;
            DBSCost.ReadOnly = true;
            DBSCost.Location = new Point(/* Specify the X and Y coordinates */);
        ...
        private async Task LoadItemPriceInformation()
        {
            try
            {
                ...
                {
                    string apiUrl = "https://api.guildwars2.com/v2/items/88771";
                    ...
                    DBSItemName.Text = $"{itemName}";

                    DBSCost.Text = $"{chatLink}, Price: {gold} Gold, {silver} Silver, {copper} Copper";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> GetItemPriceCopper()
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    string jsonResult = await client.GetStringAsync("https://api.guildwars2.com/v2/commerce/prices/88771");
                    JObject resultObject = JObject.Parse(jsonResult);
        ...
        {
            Clipboard.SetText(DBSCost.Text);
            BringGw2ToFront();
        }
    }
}