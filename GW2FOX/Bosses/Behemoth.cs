...
namespace GW2FOX
{
    public partial class Behemoth : BaseForm
    {
        ...
        private void InitializeItemPriceTextBox()
        {
            ...
            BeheCost.Text = "Item-Preis: Wird geladen...";
            BeheCost.AutoSize = true;
            BeheCost.ReadOnly = true;
            BeheCost.Location = new Point(/* Specify the X and Y coordinates */);
        ...
        private async Task LoadItemPriceInformation()
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    BeheItemName.Text = $"{itemName}";

                    BeheCost.Text = $"{chatLink}, Price: {gold} Gold, {silver} Silver, {copper} Copper";
                }
            }
            catch (Exception ex)
            {
