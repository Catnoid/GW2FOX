...
namespace GW2FOX
{
    public partial class JadeSea : BaseForm
    {
        ...
        public JadeSea(string origin) : this()
        {
        ...
        private void InitializeItemPriceTextBox()
        {
            ...
            JadeCost.Text = "Item-Preis: Wird geladen...";
            JadeCost.AutoSize = true;
            JadeCost.ReadOnly = true;
            JadeCost.Location = new Point(/* Specify the X and Y coordinates */);
        ...
        private async Task LoadItemPriceInformation()
        {
            try
            {
                ...
                {
                    string apiUrl = "https://api.guildwars2.com/v2/items/98002";
                    ...
                    JadeItemName.Text = $"{itemName}";

                    JadeCost.Text = $"{chatLink}, Price: {gold} Gold, {silver} Silver, {copper} Copper";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> GetItemPriceCopper()
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    string jsonResult = await client.GetStringAsync("https://api.guildwars2.com/v2/commerce/prices/98002");
                    JObject resultObject = JObject.Parse(jsonResult);
